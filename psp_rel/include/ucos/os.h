...
typedef enum {
	...
	MQ_ID_BT,/*蓝牙后台消息队列*/
	MQ_ID_SYS,/*系统消息队列*/
	MQ_ID_HOTPLUG,/*设备热拔插消息队列*/
	MQ_ID_GUI,/*GUI消息队列*/
	MQ_ID_GUI_TEST,/*专用于自动化测试GUI消息队列*/
	MQ_ID_MAX
} mq_id_e;
...
#define OS_Q_MSG_MANAGER_PRIVATE_LEN 2
#define OS_Q_MSG_DESK_PRIVATE_LEN    3
#define OS_Q_MSG_PRIVATE_LEN         4
...
extern unsigned short os_sem_accept(os_event_t *pevent);
#endif

extern os_event_t *os_sem_create(unsigned short cnt);
...
extern unsigned char os_sem_post(os_event_t *pevent);
...
#if OS_TASK_DEL_EN > 0
extern unsigned char os_task_del(unsigned char prio);
extern unsigned char os_task_del_request(unsigned char prio);
#endif

#if OS_TASK_SUSPEND_EN > 0
extern unsigned char os_task_resume(unsigned char prio);
extern unsigned char os_task_suspend(unsigned char prio);
#endif

#if OS_TASK_QUERY_EN > 0
extern unsigned char os_task_query(unsigned char prio, os_tcb_t *pdata);
#endif
...
extern void os_time_dly(unsigned short ticks);

#if OS_TIME_DLY_HMSM_EN > 0
#endif

#if OS_TIME_DLY_RESUME_EN > 0
extern unsigned char os_time_dly_resume(unsigned char prio);
#endif
...
extern void os_sched_lock(void);
extern void os_sched_unlock(void);
#endif
