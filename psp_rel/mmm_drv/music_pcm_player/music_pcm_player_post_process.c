 ...
 *                            All Rights Reserved.
 *
 * 描述：AUX/FM/USB播歌中间件播放后处理——数字音效设置接口。
 * 作者：cailizhen
 ********************************************************************************/
...
static uint8 __section__(".bss.echo_buf4") g_audiopp_echo_buf4[ECHO_TAIL_BUF_SIZE];
...
{
    return post_process_ops(music_player_global->audiopp_handle, PP_CMD_SET_DAE_PARA, p_info);
}

void update_PP_para_info(music_pcm_player_t *music_player)
{
    ...
    music_player->post_process_detail.sample_rate_khz = music_player->music_setting.default_samplerate;
    ...
    if (music_player->music_setting.pcm_source == PCM_SOURCE_AUX)
    {
        if ((music_player->music_setting.adc_chan_set.data_input_if == ADC_SOURCE_AUX)
            || (music_player->music_setting.adc_chan_set.data_input_if == ADC_SOURCE_MIC_AUX))
        {
            music_player->post_process_detail.ng_enable = NG_ENABLE;
        }
    }

    music_player->post_process_detail.mic_mode = MIC_MODE_DISABLE;
#if (SUPPORT_OK_MIC == 1)
    if (music_player->work_mode == PP_WORK_MODE_MIC)
    {
        music_player->post_process_detail.mic_mode = MIC_MODE_OK_MIC;
        music_player->post_process_detail.idle_mode = music_player->is_idle_mode;
    }
#endif
