...
int32 keytone_start(music_pcm_player_t *music_player, void *param)
{
    ...
    if (music_player->music_setting.default_samplerate == 44)
    {
        music_player->rs_dest_sr = 44100;
    }
    else if (music_player->music_setting.default_samplerate == 48)
    {
...
int32 keytone_stop(music_pcm_player_t *music_player)
{
...
int32 keytone_frame_deal(music_pcm_player_t *music_player, short *pcm_out, uint16 samples_count)
{
    int32 ret = 0;
...
int32 keytone_playend_deal(music_pcm_player_t *music_player)
{
    int32 ret = 0;

    libc_memset(PCM_DEC_BUF_ADDR, 0x00, MY_PCM_DEC_BUF_BLOCK);

    keytone_frame_deal(music_player, PCM_DEC_BUF_ADDR, MY_PCM_DEC_BUF_BLOCK/(sizeof(short)*2));

    send_pcm_to_pcm_buf_by_dac_fifo(PCM_DEC_BUF_ADDR, MY_PCM_DEC_BUF_BLOCK/sizeof(short));
