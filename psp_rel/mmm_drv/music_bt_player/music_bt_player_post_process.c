...
void update_PP_para_info(music_bt_player_t *music_player)
{
    ...
    aud_output_chan = music_player->music_setting.aud_output_chan;
#ifdef DEC_ENABLE_TRUE_WIRELESS_STEREO
    if (p_tws_info->dev_role != NORMAL_DEV)
    {
        if (p_tws_info->dev_spk == TWS_SPK_LEFT)
        {
            aud_output_chan = AUD_OUTPUT_LEFT_CHAN;
        }
        else if (p_tws_info->dev_spk == TWS_SPK_RIGHT)
        {
            aud_output_chan = AUD_OUTPUT_RIGHT_CHAN;
        }
    }
#endif
    ...
    music_player->post_process_detail.aud_output_chan = aud_output_chan;
