...
unsigned short jis_to_unicode(int gbk_code, FILE *in)
{
	...
	else
	{
		...
		else if (((gbk_code&0xff) >= 0xa1) && ((gbk_code&0xff) < 0xe0))
		{
			unicode_val = 0xff61 + ((gbk_code&0xff)-0xa1);
		}
...
int main(void)
{
	...
	char tbl_name[] = "V932JIS.TBL";
	char tbl_tidy_name[] = "V932JIS_TIDY.TBL";
	char tbl_large_name[] = "V932JIS_LARGE.TBL";
	int gbk_code;
	unsigned short unicode_val;
	int i;
	...
	{
		unicode_val = jis_to_unicode(gbk_code, in);
		fwrite(&unicode_val, 2, 1, out);
	}
